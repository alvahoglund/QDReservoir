...
@time sm_pure = scrambling_map(sys, measurements, ψres, hams.hamiltonian_total, t,
...
sm_pures = [scrambling_map(sys, measurements, ψres, hams.hamiltonian_total, t,
                QDR.PureStatePropagatorAlg(; krylov_dim, tol = 1e-6))
            for krylov_dim in [100, 200, 300]]
norm.(diff(sm_pures))
##
@profview sm_pure = scrambling_map(sys, measurements, ψres, hams.hamiltonian_total, t,
    QDR.PureStatePropagatorAlg(; krylov_dim = 200, tol = 1e-6));
##
@time reservoir_state = ground_state(
    hams.hamiltonian_reservoir, QDR.ExactDiagonalizationAlg())
@time measurements_total = charge_measurements(sys)
@time sm_block = scrambling_map(
    sys, measurements_total, ψres, hams.hamiltonian_total, t, QDR.BlockPropagatorAlg());
sm_block ≈ sm_pure
...
@profview sm = scrambling_map(sys, measurements, reservoir_state, ham_total,
