...
measurements = map(m -> matrix_representation(m, sys.H_total), QDR.single_charge_probabilities(sys.grid.total, sys.f))
Ω = stack(vec(QDR.hilbert_schmidt_ensemble(sys.H_main)) for i in 1:nbr_states)
S = scrambling_map(sys, measurements, ground_state(hams.hamiltonian_reservoir), hams.hamiltonian_total, [10, 100, 1000])
...
for (i, ps) in enumerate(plot_paulis)
    idx = Pm_dict[ps...]
    ax = Axis(fig[1, i], xlabel = "True value", ylabel = "Predicted value", title = "Measurement: $(ps[1]) ⊗ $(ps[2]), RMSE: $(round(sqrt(mean_squared_error[idx]), digits=4))")
