...
function default_res_dot_params(coordinates)
    ϵ_func() = rand()
    ϵb_func() = [0, 0, 1]
    u_intra_func() = rand() + 10
    return set_dot_params(ϵ_func, ϵb_func, u_intra_func, coordinates)
...
function hamiltonians(
    ...
    hamiltonian_res = hamiltonian_dots(dot_params_res, grid.res) +
